...
| 在後台 | 賓客那一頁 |
|---|---|
| **顏色**：一排色塊，點一個就跳出系統原生的顏色選擇器。還沒設定過時預設攤開 **3 格白色（#FFFFFF）**，最後面一顆「**＋**」，點了直接選色補上第四個；補滿 4 個之後「＋」就不再出現。每一個色塊右上角有 ✕ 可以移除 | 大廳的「出席前的小提醒」多一排**實心的不規則圓**，就是那幾個顏色（四顆各用一組不同的圓角，排在一起不像複製貼上）。邀請函的「服裝」那一列也帶同一組色塊 |
