...
- **首頁**：固定背景（一張圖或一段影片，滾動時不動）→ 置中開場（`h1` + `.cn`）→
  ...
  **Gallery 照片迴廊** → 日期倒數 → RSVP → 卡片連結（兩欄，內建七張＋
  新人在後台自訂的卡片）。
...
- **出席回覆**（`/w/{slug}/invitation`）走的是同一套版型 ——
  ...
  **那一頁只做一件事：收下回覆** —— 婚禮資訊、兩人的故事、照片迴廊都在首頁，
...
sites/{siteId}
  ...
  rsvpShowGallery(bool)         # 首頁要不要放「Gallery 照片迴廊」
...
所以它是首頁上**自己的一段**（`Gallery／照片迴廊`，排在兩人的故事之後、
倒數之前）：兩欄錯落往下走，捲到哪一張才載哪一張，進到畫面裡才淡入 ＋
往上浮一小段，同一批進來的依序錯開。點任何一張原地放大。

開關在「婚禮資訊 → 自訂內容」最上面的「照片迴廊」那一區，**按下去就存**
