...
| 這一塊 | 內容 |
|---|---|
| 開場 | 這個後台是什麼、賓客會看到什麼。右上角兩個出口：**查看目前網站**（賓客現在打開連結看到的樣子）與**查看表單**（賓客要填的那一頁） |
| 從這裡開始 | 有順序的幾步，每一步右邊一顆直達那一頁的按鈕。**已經填好的自動打勾**（判斷是拿資料本身算的，不另外記進度 —— 換一台裝置也不會歸零），沒開的功能不會出現在清單裡 |
| 還有這兩個地方 | 兩張卡指到側欄最下面的「**頁面設定**」與「**常見問題**」—— 兩者原本是首頁最下面的兩段，現在各自是一顆分頁 |
...
搬出首頁的理由只有一個：**埋在五步上手指南下面的東西，要用的時候找不到**。
婚禮當天要把抽卡打開的人，不會記得那個開關在首頁最底下；
卡住想找答案的人，也不會想到要回首頁往下捲。
首頁最後留兩張卡指過去，第一次照著五步走完的人才接得上下一步。
