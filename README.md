...
| `/butler#{代號}` | 收禮小幫手（婚宴當天收禮金／發禮餅）**和站台網址無關** |
| `/s/{code}` | 短連結 |
...
- **導覽列**：每一頁最上方都有，依序是「新人名稱（首頁）、出席回覆、桌次、祝福、
  ...
  還沒留下名字的訪客，最後那塊 User 會換成一顆很輕的「留下名字」
  （按了就是同一張底部問名字的卡片）。
  **開到 4 個功能以上，手機（≤720px）會收成漢堡＋左側抽屜**
  （`NAV_DRAWER_MIN`），列上改成「新人名稱／當前頁名」。
  原因是量過的：七項要 312px，手機實際分得到的只有 135–229px，
  第 4 項開始就得橫捲 —— 而捲軸是藏起來的，看不到的那幾項賓客根本
  不知道存在，連結熱區又只有 31.5px，擺在橫捲容器裡手指一斜就變成捲動。
  抽屜裡每一項都是 44px 的整條熱區。3 項以內照舊橫排，桌機永遠橫排。
- **入場登入**：**預設關著** —— 賓客一點進來就是大廳，門口不擋人。
  需要署名的動作（寫祝福、投一封信、送甜點）才在按下送出的那一刻，
  從畫面**下方**升起一張小卡片問名字（`common.js` 的 `askName()`）。
  想回到「先報上名來才進得去」的入場畫面（`#gate`），把站台文件的
  `entryLoginEnabled` 設成 `true`，見〈入場登入〉。
...
- **首頁**：固定背景（一張圖或一段影片，滾動時不動）→ 置中開場（`h1` + `.cn`）→
  婚禮資訊卡 → 當日流程 → Dress Code → 交通資訊 → 兩人的故事 →
  **Moments 相遇之間** → 日期倒數 → RSVP → 卡片連結（兩欄，內建七張＋
  新人在後台自訂的卡片）。
...
- **出席回覆**（`/w/{slug}/invitation`）走的是同一套版型 ——
  ...
  只多一份放專屬元件（封面、hashtag）的 `css/invitation.css`。
  它原本自帶一整套 CSS／JS，和其他頁長得完全不一樣，現在已經收斂進來。
  **那一頁只做一件事：收下回覆** —— 婚禮資訊、兩人的故事、「相遇之間」都在首頁，
  在那裡再放一次只會把要填的東西往下推。送出後的結果卡上有
  「回婚禮大廳」與「看我們的故事」兩個出口。
...
│   ├─ invitation.html        # 出席回覆（對外分享的那個連結，只放表單）
│   ├─ butler.html            # 收禮小幫手（婚宴當天收禮金／發禮餅，獨立網址 /butler）
...
│       ├─ butler.js          # 收禮小幫手本體（自己一支，不吃 site-context）
│       ├─ butler-key.js      # 收禮簿位置的推導（butler.html 與 admin.html 共用）
...
sites/{siteId}
  ...
  entryLoginEnabled(bool) # 大廳入場登入的總開關，新人改不動；**沒這個欄位＝關**
                          # 關（預設）：一進來就是大廳，要署名的動作才當場問名字
                          # true    ：回到「先填名字才進得去」的入場畫面
  ...
  rsvpMailEnabled(bool)         # 要不要提供「郵寄」（喜帖怎麼給、喜餅怎麼領）
  rsvpContactMethods(string[])  # 要問哪幾種聯絡方式（phone/line/email）
  rsvpShowGallery(bool)         # 首頁要不要放「Moments 相遇之間」
  rsvpShowStory(bool)           # 舊欄位，前端已不再讀
                               # 以上沒設定過一律視為「開著」，舊站台不受影響
  pages(map)            # 每個頁面開關，如 { wall:true, cake:false, … }
                        # 另外含一個沒有網址的後台功能開關 seatingPlan（排桌管理）
                        # ★ 這一份是「我們開了哪幾頁」，新人改不動
  pagePublish(map)      # 「這一頁現在公開了沒」——**新人自己**在後台首頁的
                        # 「頁面設定」開關／排程：key → { on(bool), at(number|null) }
                        # 沒有這個欄位、或某一頁沒有那一筆＝公開；
...
「後台要不要長出那個分頁」＋「規則放不放行收禮紀錄的寫入」。

#### 新人自己也能收起某一頁（後台的「頁面設定」分頁）

`pages` 是**我們**開了哪幾頁（他買了什麼），新人改不動。
但「買了」不等於「現在就要讓賓客看到」—— 故事牆的照片還沒挑完、
小測驗想留到婚宴當天才開。所以新人在後台側欄最下面的
「**頁面設定**」有自己的一層開關：一列一頁、一顆 toggle，
旁邊還可以排一個時間讓它自己開（存在 `sites.pagePublish`）。

| | `pages` | `pagePublish` |
|---|---|---|
| 誰改得動 | 只有我們（`set-pages`／Console） | 新人自己 |
| 規則讀不讀 | 讀（決定寫入放不放行） | **不讀**，只影響賓客看不看得到入口 |
| 沒設定時 | 全開 | 公開 |
| 後台 | 沒開的分頁掛鎖頭 | 收起來的頁面照樣編輯得動 |

所以我們這一側完全不用管它：新人把某一頁收起來時，
`pages` 沒有被動到，`set-pages` 印出來的也還是我們開的那一份。
桌次是唯一沿用舊欄位的（`seatingFeatureEnabled`），
後台兩個地方的開關指的是同一件事。

#### 「關」有兩種

關掉的功能在新人後台不一定是消失，要看它**開賣了沒**：

| | 新人在後台看到 | 什麼時候用 |
|---|---|---|
| **可加購**（預設） | 側欄留著、掛一顆鎖頭，點得進去看模糊的預覽與說明卡 | 功能做好了，只是這組新人還沒買 |
| **還沒開放** | 整顆收起來，完全看不到 | 功能還在做／還沒開賣 |

分界是 `scripts/site-pages.js` 的 `UNRELEASED_PAGES`
（瀏覽器那邊是 `public/js/site-context.js` 的 `UNRELEASED_FEATURES`，
兩份要一起改）：
...
`--enable` 打開之後它就是開著的，這份清單只管「沒開的時候要不要被看見」。
`npm run check-site` 與 `set-pages` 都會把兩種關法分開印出來。
...
# 打開「收禮小幫手」（婚宴當天收禮金／發禮餅的獨立小工具）
npm run set-pages -- --slug ginny-one-20260919 --enable butler

# 打開／關掉「入場登入」（賓客要不要先報上名來，**預設是關的**）
...
以前這裡有一道門：第一次打開大廳要先填名字、抽一個記號才進得去（`#gate`）。
問題是**絕大多數賓客第一件事只是想看時間、地點、找自己的桌次**，
這些事一件都不需要名字 —— 先擋一道「輸入你的名字」，
等於在人還沒決定要不要留下的時候先要他表態，
而那正是最容易直接關掉頁面的一刻。
...
| 時機 | 做法 |
|---|---|
| 進大廳 | 不問。直接播開場字幕 → 開幕簾 → 大廳（留過名字的賓客連字幕都跳過，他來過了） |
| 逛桌次、邀請函、抽卡、測驗、故事 | 不問，本來就不需要名字 |
| 寫祝福、投一封信、送甜點 | **按下送出的那一刻才問**（`common.js` 的 `askName()`），填過一次就記在 localStorage，之後不再打擾 |
| 想主動署名 | 導覽列右邊有一顆很輕的「留下名字」（`#navSignInBtn`），按了就是同一張卡片 |

#### 問名字的那張卡片長什麼樣

是一張**從畫面下方升起的 bottom sheet**（`.id-sheet`），不是蓋掉整頁的對話框。
手機貼齊螢幕底，桌機在下緣置中。三件事是刻意的：

- **看得到後面**：沒有壓黑整頁的遮罩，只在底部鋪一層很淡的漸層，
  讓卡片浮得起來而已。賓客剛剛在看的祝福牆還在原地。
- **只佔下方**：上半部的閱讀區完全不動，手機上鍵盤跳出來也不會把輸入框頂掉。
- **不鎖畫面**：外層是 `pointer-events:none`，底下照樣捲得動、點得到。
  想先滑上去確認自己在留言給誰，再回來填名字，都可以。
  不想填就按「再等等」、右上角 ✕ 或 Esc，剛剛打的字會留在原地。

卡片上有三條路：直接打名字、用 Google 帶入名字、再等等。
Google 那條用 `linkWithPopup()` **接在賓客現有的匿名帳號上**，
uid 不變，之前抽到的卡不會因為登入就消失。

#### 想要「先報上名來」的站台

有些新人就是想要一本簽到簿 —— 那就把門打開：

```bash
# 打開（回到 #gate：填名字、抽記號才進得了大廳）
npm run set-pages -- --slug ginny-one-20260919 --entry-login on
...
把 `entryLoginEnabled`（boolean）設成 `true`。
**沒有這個欄位＝視為關著。**

打開之後：大廳出現入場畫面，填完名字才播開場字幕；
子頁（祝福牆、抽卡…）也會把還沒報到的賓客請回大廳（`requireUser()`）。
...
> 側欄最上面是**首頁**（見下面〈後台首頁〉），底下十個固定分頁：
> **表單設定**、**出席回覆**、**悄悄話**、**婚禮資訊**、**桌次**、
> **感謝信**、**婚禮小卡**、**新人故事牆**、**新人熟悉測驗**，
> 另外兩個要另外打開：**排桌管理**與**收禮小幫手**。
> 「表單設定」排在「出席回覆」**上面** —— 先決定要問什麼，才會有回覆可以看。
> 手機上分頁列可以左右滑（捲得動的那一側邊緣會淡掉，就是還有東西的意思）。
> 出席回覆、桌次、新人熟悉測驗底下還有橫向子分頁（例如出席回覆分成
> 「出席回覆總覽」「回覆資訊」）；「自訂內容」是側欄自己的一頁。
> **婚禮資訊沒有子分頁**，它是一個總覽 ＋ 六個階段（`#lobby/01`…`#lobby/06`），見下面〈婚禮資訊〉。
>
> **分頁會跟著這組新人開了哪些頁面**，而「沒開」有兩種：
>
> | | 新人在後台看到 | 用在什麼時候 |
> |---|---|---|
> | **可加購** | 留在側欄、右邊掛一顆鎖頭。點得進去，內容區蓋一張說明卡，底下是模糊的預覽 | 功能已經做好、只是這組新人還沒買 |
> | **還沒開放** | 整顆收起來，完全看不到 | 功能本身還在做／還沒開賣 |
>
> 分界是 `public/js/site-context.js` 的 `UNRELEASED_FEATURES`（`scripts/site-pages.js`
> 有同一份給 CLI 用的 `UNRELEASED_PAGES`）：**代號放進去＝還沒開放，沒放＝可加購**。
> 那是「這個功能開賣了沒」的事實，對所有站台都一樣，所以寫在程式裡，
> 不是每個站台各自的設定；開賣的那天把代號刪掉就好，站台資料一個都不用動。
> 鎖著的分頁不會辛苦上傳完才發現賓客看不到 —— 它連資料訂閱都省下來，
> Security Rules 那邊本來就不放行。
> 出席回覆與表單設定↔`rsvp`（同一個開關）、桌次↔`seating`、感謝信↔`letter`、婚禮小卡↔`draw`、
> 新人故事牆↔`exhibition`、新人熟悉測驗↔`quiz`、
> 排桌管理↔`seatingPlan`、收禮小幫手↔`butler`；
> 「首頁」「婚禮資訊」與「首頁卡片」永遠都在。
...
後台不是「桌機版的縮小版」。新人真正在用它的時間多半是通勤、睡前躺著、
婚宴當天站在會場裡，所以這幾件事是刻意做的：

- **左上角的漢堡鈕**打開分頁抽屜，抽屜左上角有 ✕ 可以關掉，
  按手機的**返回鍵**也會關掉抽屜或彈窗，**不會**直接離開後台
  把你填到一半的表單丟掉。
- **彈窗從畫面底部升起來**，上緣有一條短短的握把，從那裡往下滑就推走了。
  「儲存」永遠黏在彈窗最下面，就算鍵盤升起來也看得到。
- **回覆名單與收禮明細在手機上是一位一張卡**，不是要左右滑三四個螢幕的表格。
  最常看的那幾個數字（幾位、葷幾素幾、兒童椅幾張）排在第二行，
  其餘收進「展開更多」。桌機維持表格（同一欄上下對得起來才比得出差異）。
- **「刪除」不會和「編輯」並排**：手機上收進每一列右邊的 **⋮**，
  排在最後、紅字，不會被拇指掃到。可以排序的清單（測驗題目、新人故事牆、
  婚禮小卡、桌次圖、桌位）也在同一顆裡：上移／下移／移到最前／移到最後。
  **婚禮小卡的「重新裁切」與「刪除」只收在這一顆裡** ——
  卡片本身只有 180px 寬，兩顆文字按鈕擠在同一行很容易點錯。
- **「儲存」會浮在畫面最下面**（婚禮資訊的每一個階段、表單設定），
  有還沒存的變更時旁邊會寫「有還沒儲存的變更」，切到別的分頁也會提醒一聲。
  改到一半關掉分頁的話，下次進來會問你要不要接回上次沒存完的內容。
- **離線的時候畫面會告訴你**：頂列底下壓一條「目前離線，改動不會存進去」。
  按了儲存但網路卡住的話，按鈕會停在「儲存中…」最多 8 秒，
  然後跳出「網路好像不太穩，這一筆還沒送出去」＋一顆「重試」——
  資料仍然排在手機的佇列裡，連線回來會自己補送，不會不見。

> **iPad 橫向**也算觸控裝置：雖然螢幕跟筆電一樣寬，
> 但排桌那一頁不會給你「拖得動」的假暗示（拖曳在觸控上完全不會觸發），
> 一律走「⇄ → 選桌位」那條路。
...
新人第一次登入時，看到的是十顆他沒看過的分頁名稱，沒有一句話說「先做哪一個」。
首頁補上那句話，也是**頂列「新人後台」點下去會回到的地方**（和一般網站點 logo 回首頁一樣）。

| 這一塊 | 內容 |
|---|---|
| 開場 | 網站已經做好了，接下來要放什麼內容都在這裡改。右上角兩個出口：**查看婚禮網站**（賓客現在打開連結看到的樣子）與**查看表單**（賓客要填的那一頁） |
| 給你的製作筆記 | 一張**預設收起來**的卡（`<details>`），只露出標題與副標題。展開是**建議的節奏**：拿到後台的第一週／有空的時候慢慢做／婚禮前一到兩週／婚禮前一天／結束後 —— 下面那幾步講「做什麼」，這一段講「什麼時候做」，所以擺在步驟前面：先給節奏，再給清單 |
| 開始製作 | 有順序的幾步，每一步右邊一顆直達那一頁的按鈕。**已經填好的自動打勾**（判斷是拿資料本身算的，不另外記進度 —— 換一台裝置也不會歸零），完成訊息接在標題後面省一行，沒開的功能不會出現在清單裡 |
| 還有這兩個頁面 | 兩張卡指到側欄最下面的「**頁面設定**」與「**常見問題**」—— 兩者原本是首頁最下面的兩段，現在各自是一顆分頁 |

第一步是唯一的例外：「**把後台放到手機主畫面**」不跳分頁，
就在那一列裡展開 iPhone 與 Android 的操作步驟，按「**我已經完成**」才收起來。
它問的是「這一台裝置做過了沒」，資料庫裡沒有答案，
所以打勾記在 localStorage（`HOME_PIN_KEY`，以 siteId 分隔）——
換一台手機就該再問一次，因為那台確實還沒放。

#### 側欄最下面的兩顆分頁

「頁面設定」與「常見問題」和「首頁」同一層（都不是在編輯某一塊賓客內容），
所以不進婚禮管理／婚禮內容／賓客互動任何一組，用一道淡線隔在側欄最底下。

搬出首頁的理由只有一個：**埋在「開始製作」那幾步下面的東西，要用的時候找不到**。
婚禮當天要把抽卡打開的人，不會記得那個開關在首頁最底下；
卡住想找答案的人，也不會想到要回首頁往下捲。
首頁最後留兩張卡指過去，第一次照著走完的人才接得上下一步。

| 分頁 | 內容 |
|---|---|
| **頁面設定** | 一列一頁，右邊一顆 toggle 決定**賓客現在看不看得到**；旁邊的「排程開啟」可以設一個時間讓它自己開。右邊一支手機示意賓客現在打開首頁看到的入口（**收起來的就不會出現在上面**） |
| **常見問題** | 十四則問答，分成五類（開始使用／頁面與功能開關／照片與內容／出席回覆與名單／婚禮當天），最下面一塊是 **LINE 官方帳號** —— 這一頁回答不了的，一鍵去問人 |

官方帳號的網址全站只寫在 `public/js/admin.js` 的 `SUPPORT_LINE_URL` 一個地方
（常見問題那顆按鈕、頁面設定裡沒開通那幾列的說明，讀的都是它），
換帳號只改那一行。

#### 頁面設定怎麼運作

- **一列一頁**：祝福牆、給你的信、我們的故事、小測驗、抽卡、桌次…
  清單是從 `public/js/site-context.js` 的 `PAGES` 長出來的，
  **之後新增的頁面會自己出現在這一區**，不必回來改一份清單
- **沒開通的頁面是灰的、掛一顆鎖頭**，toggle 按不動（那是我們那一層的開關）；
  還沒對外開放的功能整列不出現
- **按不動的那一列會自己說明白**：開關旁邊有一顆**問號**——
  桌機滑過去就看得到，手機點一下 —— 展開「這個功能需要管理員才能開啟，
  請透過官方帳號聯繫」，加一顆直接連到 LINE 官方帳號的按鈕，
  外加一句「現有的資料一筆都不會動」。
  直接按那顆按不動的開關也會展開同一段（死掉的開關按下去什麼都不發生，
  新人只會以為是壞的），而且多吐一則 toast
- **收起來 ≠ 資料不見**：後台照樣編輯得動，賓客那邊只是沒有入口、
  直接打網址會被帶回首頁
- **排程沒有後端**：時間到了不會有任何程序去改資料，而是每個賓客開頁面時
  重新判斷一次「現在過了那個時間沒有」。所以新人排完就可以關掉後台
- **打開或關掉都會順手清掉排程** —— 不清的話「我明明關起來了」
  過幾小時又自己開回來
- **桌次沿用舊欄位** `seatingFeatureEnabled`：「桌次」分頁上那一顆開關
  和這裡這一列講的是同一件事，兩邊怎麼按都同步

---

### 0b. 表單設定（後台「表單設定」分頁）

決定 `/w/{slug}/invitation` 那一頁要問什麼、放什麼，
右上角的**「查看表單」**直接開賓客看到的那一頁（就是分享出去的網址）。

> 這一頁原本是「出席回覆」底下的第三顆子分頁，實測時新人找不到 ——
> 他要找的是「我的表單長什麼樣」，那不是「已經收到的回覆」的一部分。
> 現在它是側欄自己的一顆，而且排在「出席回覆」上面。
> 舊網址 `#rsvp/form` 仍然會自動帶到新的位置。

| 設定 | 內容 |
|---|---|
| 題目 | 依表單順序列出目前所有題目。喜帖發送方式／喜餅領取方式／想對新人說的話可以個別關掉；**固定題目用打勾但關不掉（disabled）的勾選框列出來**，讓新人一眼看到表單上實際會有哪些題目 |
| 要不要提供「郵寄」 | 關掉的話，喜帖的「紙本要怎麼給你？」與喜餅的領取方式各少一個**郵寄**，也就不會問地址。不寄送的新人不該讓賓客填了地址才發現沒有這回事 |
| 聯絡方式 | 電話號碼／LINE ID／Email，**可複選**；勾了幾種，賓客就要至少填其中一種 |
| 封面那一段 | 表單上面那一段會出現的內容（名字、日期與倒數、hashtag），只是列出目前填了什麼，按「去填寫婚禮資訊」跳到那一頁 |

> **那一頁只剩出席回覆**：地點、交通、Dress Code、禮金、兩人的故事與「相遇之間」
> 都只出現在首頁，出席表單不再放第二次 —— 會拿到那個連結的人是來回覆的，
> 那幾塊擺在表單前面只會把要填的東西往下推。
> 賓客送出之後，結果卡上有「**回婚禮大廳**」與「**看我們的故事**」兩個出口。
> 照片的開關在「婚禮資訊 → 06 你們的故事」（它現在是首頁上自己的一段，
> 見〈Moments 相遇之間〉）；照片本身放在素材資料夾（見〈素材資料夾〉），
> 後台不能自己上傳。
...
收到的回覆名單，畫成一張**表格**：一列一筆回覆，一欄一件事
（標籤、姓名、出席回應、分類、聯絡資訊、人數、葷、素、兒童椅、飲食習慣、
喜帖、喜餅、給新人的話、備註、填表時間）。同一欄從上到下對得起來，
才比得出「誰吃素、誰要寄喜餅」。欄位多到一定會橫向捲，所以**表頭固定在上面**，
捲到第 40 列還看得到自己在看哪一欄。**關掉的題目整欄不出現** ——
一整欄的「—」沒有任何資訊。

最上面是一條篩選列，出席狀態、標籤、搜尋收在同一塊裡：

| 這一排 | 做什麼 |
|---|---|
| 出席 | 全部／熱情出席／視情況而定／無法出席 |
| 標籤 | 全部標籤／各個標籤／沒有標籤，最後一顆是「**設定標籤 ↗**」，直接跳去改標籤 |
| 搜尋 | 名字、留言、備註、標籤名字都吃得到 |

右上角是**匯出 CSV**（欄位與 `npm run export-rsvps` 一致，含 BOM，
Excel 開中文不會亂碼；匯出的是「目前篩選出來的那些」，不是全部）。

#### 刪掉重複的回覆

同一個人手滑送出好幾份是很常見的事，所以每一列右邊有「**刪除**」。
回覆的**內容仍然改不動**（規則擋 update），能做的只有把整筆拿掉。

要連過兩關：第一關把那筆的狀態、人數、送出時間列出來讓你看清楚，
第二關要**打字輸入「刪除」**才按得下去。

> **編號不會重複使用。**排桌管理的自動編號（A01、B02…）是照回覆順序算的，
> 刪掉一筆本來會讓後面每個人往前挪一號 —— 但新人可能已經把 B06
> 寫在紙本名單、桌卡上了。所以刪除前會先把現在的編號**釘住**：
>
> - 原本 B01～B08，刪掉 B05 → 其他人**原地不動**（B01～B04、B06～B08）
> - 下一筆新回覆拿到 **B09**，不會把 B05 撿回去用
>
...
給新人自己把賓客分群用的。按「加入常用標籤」可以複選帶進來：**長輩、行動不便、
孕婦、同事、大學同學、高中同學、國小同學、小孩**（已經有同名的會標「已加入」
並停用），也可以自己新增。**一位賓客可以有好幾個標籤。**

設定那一頁是**兩組 chip**：

- **賓客表單自選標籤** —— 賓客填回覆時可以自己勾的
- **後台管理標籤** —— 只有新人和工作人員看得到

一顆 chip 上就是「名稱 ＋ 共 N 位 ＋ 一支鉛筆」，點哪裡都是打開底部彈窗
（新增與編輯共用同一個）：改名、用 radio 換組、刪除。**換組就是搬一顆 chip，
不用刪掉重建**；已經有人套用時彈窗會先說一句「修改分類不會影響他們的資料」。

| 在哪裡 | 可以做什麼 |
|---|---|
| 婚禮內容 →「設定賓客標籤」 | 新增／改名／換組／刪除標籤，看每個標籤用在幾位賓客身上。這是側欄自己的一頁，沒開標籤功能的站台那一頁會掛鎖頭 |
| 賓客那一頁 | 「與新人的關係」下面多一題**單選、選填**的「更具體是哪一種？」，選項就是放在「賓客表單自選標籤」那一組的標籤 |
...
> **後台改不動回覆的內容。**
> 規則對 `rsvps` 開放的是 `read` 與 `delete`，`update` 仍然是 `false` ——
> 回覆是賓客送出的紀錄，不該在後台被改掉。
> 重複送出的那幾筆可以整筆刪掉（「回覆資訊」那一頁，要連按兩次確認），
> 留下來的每一筆都還是賓客當初寫的原文。
...
**只顯示自己的桌號** —— 名單是新人整理的資料，
查自己的位子不必連帶看到同桌其他人的名字。

**整個功能要不要開**（後台「桌次 → 桌次圖」最上面那一顆，
也在**後台的「頁面設定」分頁**列了一列，兩邊是同一顆開關）：
關掉時大廳不出現「尋找我的座位」、導覽列也沒有桌次。
在「頁面設定」那一列還可以**排一個時間**讓它婚禮當天自己開。
...
整頁只講一句話：**「還沒坐下的人在左邊，可以坐的桌子在右邊。」**

三個子分頁：

| 子分頁 | 做什麼 |
|---|---|
| 排桌工作區 | 左邊「待安排」、右邊「我的桌位」，拖曳就能排 |
| 桌位管理 | 新增／刪除／改桌名、桌號、容量、類型、順序 |
| 匯入匯出 | 「匯出」在上面（名單排好之後最常做的事），底下才是「從出席表單匯入」與 Excel／CSV 匯入 |

工作區最上面永遠看得到三個數字 —— **待安排**（幾組）、總人數、總桌數 ——
接著是發布狀態與「儲存／發布」，再來是提醒，最後才是左右兩欄。
統計刻意只留三個：沒有人是看著「平均一桌幾人」決定誰坐哪一桌的，
那個數字只會把真正要盯的「待安排」擠小。還有人沒排時，那一格整格轉成警示色。
**「待安排」那一欄是黏著的**，捲到第 24 桌也還看得到還有誰沒排。

#### 沒有資料的時候也是同一副骨架

第一次進來不會看到一頁「四個步驟」的教學。**版面完全一樣**，
只是把缺掉的那一塊換成一張提示卡，長在它本來就該在的那一欄裡：

| 現在的狀況 | 左邊（待安排） | 右邊（我的桌位） |
|---|---|---|
| 什麼都還沒有 | 「你想如何匯入賓客名單？」 | 同一張 ＋「當天預計有幾桌呢？」＋ 建立標籤的小技巧 |
| 有賓客、沒桌位 | 正常的賓客清單 | 「當天預計有幾桌呢？」 |
| 有桌位、沒賓客 | 「你想如何匯入賓客名單？」 | 正常的桌位 |
| 都有，還沒排 | 正常的賓客清單 | 正常的桌位 |
| 全部排完 | ✓ 沒有人在等位子了 | 正常的桌位 |

提示卡刻意**不編號**：先建標籤、先匯名單、先開桌子都可以，
寫成「第一步／第二步」只是在替新人決定順序。
...
桌卡上寫的是**容量與空位**，不是「現在坐了幾個」——
排桌時要判斷的是「這一桌還放不放得下」，「3 / 10 人」要新人自己減一次才有答案。

| 桌況 | 桌卡上寫 |
|---|---|
| 容量 10、已坐 8 | `10 人・剩 2 位` |
| 容量 10、已坐 10 | `10 人・已滿`（綠框） |
| 容量 10、已坐 12 | `10 人・超過 2 位`（紅框） |
...
桌位類型有主桌／親友桌／同學桌／同事桌／預備桌／自訂（舊資料的家人桌、VIP 也還在）。
它講的是「這張桌子的用途」，和賓客標籤（「這位賓客的特徵」）是兩件事。

#### 一次加好幾桌

「新增多個桌位」不是問「你要加幾桌」，而是**照類別一列一列問**：
...
新人心裡想的從來不是一個總數，而是「主桌一桌、同學大概三桌、同事兩桌」。
上面會先算好：「目前 33 人，10 人桌大約需要 4 桌。會從第 01 桌接著編號，
容量預設 10 人，之後都可以改。」主桌預設 1，其餘把建議桌數平均分掉，
預備桌留 0（那是「怕人多」才加的，不該替新人決定）。
每一列都可以按加減鈕，也可以直接打字；「＋ 自訂類別」可以自己開一列（招待桌、廠商桌…）。
...
**手機／平板**不複製這一套（HTML5 的拖曳在觸控裝置根本不會觸發）：
每張賓客卡右邊直接有一顆 **⇄**，按下去是一張列出所有桌位
（現在幾人、會不會超過）的清單，選一下就座位就換好了。
點卡片本身則是展開那個人的完整資料。
...
第一行是「誰、幾位」（排桌時要盯的），第二行是「對紙本名單用的編號」與標籤。
兩欄的格子在 390px 的螢幕上只有一百五十幾 px 寬，
原本一整行要塞「編號 ＋ 姓名 ＋（N 人）」、右邊還要讓出位置給 ⇄，
留給名字的只剩二十幾 px —— 中文名字只看得到一個字加刪節號。
排桌從頭到尾都是在對名字，那是這張卡上唯一不能被犧牲的東西。

畫面最下面有一條**固定的底列**：上面一行是「現在存了沒、發布了沒」，
下面是 **⋮**、「N 待安排」（點下去回到待安排區）與**儲存**。
右邊的 **⋮** 收著發布、復原、重做與三個統計。
一場 30 桌的婚禮要按幾十次儲存，那顆按鈕不該每次都要捲回最上面找；
狀態那一句也一樣 —— 它本來只寫在頁面最上面，手機上等於看不到，
「已經改好了但沒存」就這樣被帶去婚宴現場。

> 底列有了之後，**頂列那一份狀態與那四顆按鈕在 ≤960px 就收起來**：
> 同一句話在同一個畫面上講兩次，第二次不會讓人比較放心。

> **排到一半被打斷也不會不見**：每次有改動就會在這台裝置上留一份草稿
> （切到別的 App、手機自己把分頁回收掉都來得及存）。
> 下次進來如果那份草稿比雲端新，會問你「上次有一份還沒存的排桌，要接續嗎？」。
> 說「重新開始」也**不會**把草稿刪掉 —— 誤觸的代價太高了。
...
點賓客卡（桌機直接開，觸控是先展開完整樣貌、再按「賓客資料」）會開一個**彈窗**。
後台其他「看一筆的完整內容」都是彈窗，這一頁本來是唯一的抽屜 ——
同一件事有兩種開法，使用者就得學兩次。

裡面的欄位盡量是「選的」而不是「打字的」，因為排桌從頭到尾都在分組，
而打字打出來的「女方朋友」「女方的朋友」永遠分不到同一組：

| 欄位 | 長怎樣 |
|---|---|
| 編號 / 顯示名稱 | 一般輸入框（出席回覆來的賓客姓名改不動） |
| 類別 | 出席表單上那四個固定選項（男方親友／女方親友／雙方親友／其他） |
| 會不會來 | 會來／保留席（還不確定）／無法出席 |
| 人數・兒童椅・喜餅 | `−  N  ＋`（也可以直接打字） |
| 餐點 | 葷食／素食 |
| 標籤 | 和「設定賓客標籤」那一頁同一組 chip，分成「賓客自己選得到」「只有你們看得到」兩群，點一下就切換 |
| 備註・目前桌位 | 照舊 |

> 兩種標籤後台改不動，會掛一把 🔒：賓客自己在表單上選的那一個，
> 以及從回覆推出來的（葷素分配填了素食、要了兒童座椅）。
>
> 餐點那一組**沒被動過就不會寫進草稿**。出席回覆可能填的是
> 「3 位裡面有 1 位吃素」，直接存一個「素食」會把那個數字蓋掉。

#### 找人與分群

九成的時候使用者只是要找一個人，所以**只有搜尋是一直看得到的輸入框**，
篩選與排序收成它旁邊兩顆圖示鈕（漏斗、上下箭頭）。

- **搜尋**：姓名、編號、類別、桌號、標籤都吃得到。打「B05」找得到那個人，打「王小明」直接看到他在第 06 桌
- **篩選**（漏斗鈕 → 彈窗）：出席（全部／已確認／保留席）、賓客標籤（**可多選**，VIP＋素食）、特殊需求（素食／兒童座椅）、要不要列出「無法出席」
- **排序**（箭頭鈕 → 選單）：依編號（預設）／優先按照標籤分組／依姓名／人數多→少／人數少→多

套了幾個條件會標在漏斗鈕上，搜尋框底下也會寫「素食・顯示 3 組　清除」——
名單少了人卻看不出原因，是這一頁最容易讓人以為「資料不見了」的地方。

選「優先按照標籤分組」時，待安排區會依標籤分成一群一群，
排序選單裡也會多一項**標籤優先順序**：一位賓客可能同時有
「女方好友＋VIP＋素食」，由上往下第一個對到的標籤就是他的組，
...
```
🥬 2 位素食    🪑 2 個兒童椅    🔖 4 個保留席    ♿ 1 位行動不便
```

**保留席**就是「還不確定會不會來」的那幾位（本來叫「待確認」）。
他們在桌上仍然佔著位子，所以和素食、兒童椅並排 ——
它們是同一種東西：「這一桌要跟飯店講的話」。
匯出的**桌位排桌表**也會標：`王大明（保留席）`。那張表沒有 RSVP 欄，
而印出來交給宴會廳的人必須看得出「這個位子是留的、人不一定會到」。
（賓客明細本來就有 RSVP 欄，不會再標一次。）

上方的提醒區也會列出來：「還有 1 組，共 3 位賓客待安排」
「第 05 桌超過容量 2 位」「B18 還沒確認會不會來（保留席）」這一類。
**這些都只是提醒，不會擋住你**。
全部排完之後那一條會換成一句輕輕的「**✓ 所有賓客都已安排**」，
不會再留一條紅色的東西在那裡。
...
排桌（在瀏覽器裡改，可以無限復原）
  → 按「儲存」→ 存成草稿（只有你們看得到）
  → 按「發布給賓客」→ 賓客的「我的桌次」才會更新
```

本來這兩件事混在一起講（「儲存排桌」「再送一次」「送到賓客的座位位置」
三種說法在講兩件事，而最長的那一句沒有人會這樣講話）。
現在就是兩個詞、兩顆按鈕：

| 說法 | 意思 |
|---|---|
| **儲存** | 保存目前的排桌結果（只有你們看得到） |
| **發布給賓客** | 讓賓客看到最新這一版 |

> **不會自動發布**，這是刻意的。新人排桌一定會反覆調整，
> 賓客那一頁不該因為後台還在整理座位就一直跟著跳。

**主要動作最多只有一顆**，而且可能一顆都沒有。
狀態一直寫在旁邊（手機是寫在底列上，就貼在「儲存」上面）：

| # | 情境 | 徽章 | 說明 | 主要動作 |
|---|---|---|---|---|
| 1 | 全新，尚未排桌 | 尚未排桌 | 還沒有安排賓客。 | — |
| 2 | 有排桌，但尚未儲存 | 尚未儲存 | 排桌有變更。 | 儲存 |
| 3 | 已儲存，但從未發布 | 尚未發布 | 賓客目前無法查詢桌次。 | 發布給賓客 |
| 4 | 已儲存後再次修改，尚未儲存 | 尚未儲存 | 排桌有變更。 | 儲存 |
| 5 | 已發布，沒有新變更 | 已發布 | 賓客可查詢桌次。<br>8/25 16:10 發布 | — |
| 6 | 已發布 → 修改 → 儲存，尚未重新發布 | 有未發布的變更 | 賓客目前看到 8/25 16:10 發布的版本。 | 發布給賓客 |
| 7 | 已發布 → 修改，但尚未儲存 | 尚未儲存 | 排桌有變更。<br>賓客目前看到 8/25 16:10 發布的版本。 | 儲存 |
| 8 | 已發布 → 修改 → 儲存 → 再修改，尚未儲存 | 尚未儲存 | 排桌有變更。<br>賓客目前看到 8/25 16:10 發布的版本。 | 儲存 |

> **「沒存」永遠最先講**（狀態 2、4、7、8 的徽章都是「尚未儲存」）。
> 存檔是唯一一件「關掉分頁就消失」的事，其餘狀態都還有下一次機會。
>
> 狀態 1 與 5 沒有主要動作：沒有東西要存、也沒有新版本要發布，
> 這時候不該有人被叫去按什麼。兩顆按鈕都還按得動，只是都不搶眼。

發布是**整份換掉**「我的桌次」那份名單，所以後台排桌與賓客查到的
永遠是同一份資料，不會有兩套真相。

> 發布之後，「桌次」分頁的桌次名單就是排桌的結果。
> 如果你原本已經在那裡手動匯入過名單，發布會把它整份換掉（會先問一次）。
...
排桌名單的第一個來源就是**出席回覆**：賓客一送出，左邊的「待安排」就會多一位，
...
| CSV（賓客明細） | 一列一位賓客 |
| CSV（桌位排桌表） | 依桌位分組，每桌結尾附小計 |

賓客明細的欄位：類別、編號、桌號、桌名、賓客姓名、人數、RSVP、Tags、備註。

桌位排桌表一桌一段，欄位是**姓名、人數、素食、兒童椅、備註**，
每一桌結尾一列「**小計**」，最後再一列「**全部合計**」——
婚宴當天要跟飯店講的就是「第 3 桌兩份素食、一張兒童椅」，一個總數幫不上忙。
素食與兒童椅的算法和排桌板上的桌卡提醒共用同一份程式碼
（回覆填過葷素分配的用實際人數，只有標籤沒有數字的才整筆算），兩邊不會對不起來。

> Excel 的讀寫是自己寫的（`public/js/xlsx-lite.js`），沒有引入任何函式庫 ——
> `.xlsx` 就是一包 ZIP 裡的 XML，用瀏覽器內建的解壓縮與 XML 解析就夠了。
>
> ⚠️ 寫進 XML 的每一格都要跳脫 `&`、`<`、`>`：備註裡一個「王小明 & 陳小美」
> 沒跳脫，整份工作表就不是合法的 XML，Excel 打開只會說「檔案毀損」，
> 而且不會告訴你是哪一格。`tests/multipage.mjs` 會把匯出的檔案讀回來，
...
信紙下面有「**儲存下載**」，把這封信存成一張 **JPG**
（用 canvas 重畫一次同一張信紙，沒有引入 html2canvas 之類的函式庫）。
**在手機上存的是相簿、不是下載資料夾**：先叫系統的分享單，選「儲存影像」
（Android 是「儲存到相簿」）就收進相簿；分享單不支援帶檔案的瀏覽器，
會把圖攤成一張全螢幕的圖，長按存下來。桌機維持原本的下載檔案。
...
照片**不在 Explore 裡**：Explore 是「還可以去哪裡」，照片不是另一個活動，
它就是這場婚禮本身。藏在一張卡後面，等於要賓客先猜到裡面有東西才看得到。

所以它是首頁上**自己的一段**（`Moments／相遇之間`，排在兩人的故事之後、
倒數之前）：兩欄錯落往下走，捲到哪一張才載哪一張，進到畫面裡才淡入 ＋
往上浮一小段，同一批進來的依序錯開。點任何一張原地放大。

開關在「婚禮資訊 → **06 你們的故事**」的「照片集（相遇之間）」那一區，**按下去就存**
（欄位沿用 `rsvpShowGallery`，已經關掉的站台不會因為搬家又被打開）；
照片本身放在素材資料夾的 `gallery/`，後台不能自己上傳。
...
這一頁**不是一張長表單**，是一個總覽 ＋ 六個階段。原本的問題不是欄位不夠，
而是「資訊很多，但不知道應該先填什麼、為什麼要填、填完之後會影響哪裡」——
十幾組欄位平鋪在同一個平面上，捲五六個螢幕才走得完。

```
#lobby        總覽：進度圓點 ＋ 六張卡
  ├ #lobby/01  基本資料    先告訴我，你們的婚禮名稱是什麼？
  ├ #lobby/02  婚禮活動    當天會有哪些活動？
  ├ #lobby/03  時間與地點  設定每個活動的時間和地點
  ├ #lobby/04  婚禮流程    婚禮當天，流程是怎麼安排的？
  ├ #lobby/05  賓客指南    想提醒賓客什麼？
  └ #lobby/06  你們的故事  最後，留一點你們的故事
```

總覽上每一張卡都寫著現在的狀態：**已完成**／**N 個活動**／**N 個流程**／
**尚未設定**（前三段）／**選填**（後三段）。
後三段刻意不寫「尚未填寫」—— 它們本來就可以不填，那句話只會讓人以為自己漏了東西。

> **資料結構一個欄位都沒有新增。** 寫回去的仍然是 `sites/{siteId}` 上原本那幾欄
> （見 `firestore.rules` 的 `isValidSiteContentUpdate` 白名單），
> 所有輸入欄位的 `id` 也維持原樣。舊站台打開就是自己原本的內容，
> 不需要任何 migration。
...
| 欄位 | 說明 |
|---|---|
| 婚禮名稱 | 資訊卡最上面那行字，**最多 20 個字**；一打開就預先填好兩位的名字（留白也是用它） |
| 婚禮 Hashtag | 空格或逗號分開，最多 3 個；沒寫 `#` 會自動補上。**留白就用 `#我們結婚了`、`#Married`** |

#### 02 婚禮活動

不問「要建立幾個場次」，直接問**當天會有哪些活動**：
證婚／婚宴／文訂／迎娶／After Party／其他，複選。
**婚宴永遠勾著而且取消不了**（至少要留一場）。
勾完按「儲存並繼續」，系統自己把對應的活動建起來，順序照婚禮當天的先後排
（文訂 → 迎娶 → 證婚 → 婚宴 → After Party → 其他）。
已經存在的同型別活動整筆留著，id、時間、地點都不會被動到；
取消勾選要移除某一場時會先問一句（而且會講清楚已經有幾筆回覆包含它）。

> **多場次是進階功能。** `multiEventEnabled` 沒開的站台，勾第二項會被打回去，
> 並顯示「多場次為進階功能，請聯繫管理員」。
> 這不是為了推銷 —— 前台的 `weddingEvents()` 不看那個旗標，
> 只要 `events[]` 寫進去，賓客那一頁**立刻**就會變成多活動的樣子。

建好的活動在同一個階段下面列成一張一張的卡：順序用左邊的 **⠿** 拖（放開就存），
內容點「編輯」在彈窗裡改（含活動種類、結束時間、要不要請賓客回覆、本場次專屬問題）。
...
| 欄位 | 說明 |
|---|---|
| 時間 | **只改得動幾點開始**，日期要透過我們（見下面〈改不動的欄位〉） |
| 地點名稱 | 資訊卡上的大字 |
| 地址 | 地點下方的小字，也是「開啟地圖」的預設搜尋字串 |
| 地圖連結 | 選填，只收 `http(s)://` 開頭；**留白就用地址自動開 Google 地圖** |

有多個活動時改成**一個活動一組欄位**，而且第二個以後多一顆
**「和婚宴同一個地點」**（沒填過地點、或地址和主要活動一樣時預設勾著）。
勾著就不用重複輸入 —— 存下去的時候會把主要活動的地點抄過去，
所以賓客那一頁每一場都看得到完整地址。

> **「相同地點」不是新欄位**：它是從資料推出來的（地址和主要活動一樣＝勾著），
> 存的仍然是每一場自己的 `venueName`／`address`／`mapUrl`。
> 舊資料不必 migration，前台也不知道有這件事。

主要活動（婚宴）的地點會**鏡像回站台文件**的 `venueName`／`venueAddress`／`venueMapUrl` ——
分享縮圖（`build-og`）與加入行事曆讀的還是那一份。

#### 04 婚禮流程

一條讀得懂的**時間軸**（`10:00　賓客入場`），不是一疊輸入框。
要新增或修改才長出一組 inline 表單（時間／活動名稱／說明），
**新增和編輯共用同一組**，所以畫面上同一時間最多一組輸入框。
排序用左邊的 **⠿** 拖；編輯與刪除收在每一列右邊的 **⋮** 裡（刪掉有 5 秒可以復原）。

由上到下就是時間軸的顯示順序 —— **不會依時間重新排**，
所以「11:30 起」這種寫法也沒問題。一列都沒有時，大廳顯示「流程稍後公布，敬請期待」。
有填流程時，大廳資訊卡的「時間」那一列底下會多一個文字連結，點了直接捲到當日流程。

#### 05 賓客指南

先問**想提醒賓客什麼**（大眾運輸／停車資訊／Dress Code／禮金），
勾起來才展開那一段。已經有內容的項目一進來就是勾著的，舊資料不會被藏起來；
要收起一段已經寫了東西的內容時會先問一句。

| 勾了之後 | 要填什麼 |
|---|---|
| 大眾運輸 | 「大眾運輸怎麼到？」文字 ＋ 一張圖 |
| 停車資訊 | 「在哪邊可以停車？」文字 ＋ 一張圖 |
| Dress Code | 「想讓賓客穿什麼顏色？」色票（最多 4 個）＋ 一句說明 ＋ 參考圖（最多 5 張） |
| 禮金 | 「有什麼想跟賓客說禮金怎麼預備？」 |

> 交通與 Dress Code 這兩塊在賓客那一頁都是左右兩格，只填一格時那一格會佔滿整列。

##### Dress Code 的顏色與參考圖

「溫柔大地色系」四個字，十個賓客會穿出十種顏色。所以這一區可以直接把顏色與照片指出來：

| 在後台 | 賓客那一頁 |
|---|---|
| **顏色**：一排色塊，點一個就跳出系統原生的顏色選擇器。還沒設定過時預設攤開 **3 格白色（#FFFFFF）**，最後面一顆「**＋**」，點了直接選色補上第四個；補滿 4 個之後「＋」就不再出現。每一個色塊右上角有 ✕ 可以移除 | 大廳的「出席前的小提醒」多一排**實心的不規則圓**，就是那幾個顏色 |
| **參考圖**：最多 5 張，可以用 ←／→ 排順序、單張刪除。第一張標著「預設顯示」 | 卡片上**只露第一張**，其餘要點「**查看更多**」才在彈窗裡看完整的一疊 |

顏色、參考圖與交通圖片都是**選好就存**，不必等「儲存並繼續」（欄位旁邊也寫了）。
Dress Code 的三樣（文字、顏色、參考圖）**任何一樣有設定，大廳就會出現這一塊**。

> **顏色與參考圖存在不同地方**：顏色是站台文件上的 `dressCodeColors`（最多 4 個 `#RRGGBB`），
> 參考圖是子集合 `sites/{siteId}/dressImages`（一張一份文件，和桌次圖 `seatingImages` 同一套做法）。
> 五張 data URL 塞進站台文件會直接撞上 Firestore 單一文件 1MB 的上限，所以圖一定要分出去。

#### 06 你們的故事

兩人的故事（大廳的 Our Story，留白則不出現）＋「相遇之間」的照片開關。
照片本身放在**這場婚禮的素材資料夾**裡，後台只說現在有幾張 ——
要新增或刪除請聯繫管理員。

#### 儲存

每一個階段底部都是同一列：**上一步** ＋ **儲存並繼續**（06 是「儲存並完成」）。
那一顆會把整頁還沒存的東西一起存掉（表單欄位、流程、多活動的地點），
所以停在哪一階段按，結果都一樣完整。沒有變動的部分會直接跳過，不會多寫回資料庫。

離開一個階段再回來，還沒存的內容**不會不見**（草稿留在畫面上，
也每 1.5 秒寫一份進 localStorage）；切到別的分頁會提醒一聲，
下次進後台會問要不要接回上次沒存完的那一份。

06 存完會跳出**完成畫面**：列出已經準備好的項目（✓ 基本資訊、日期時間、婚禮地點、
交通資訊、婚禮流程、Dress Code），還沒填的用「**還有 N 個項目可以補充**」
（○ 關於禮金、你們的故事），底下一顆「查看我的婚禮」。

> **改不動的欄位**：新人姓名、婚禮日期、頁面開關、入場登入的開關、
> 出席回覆的開關與截止時間、多場次的總開關 `multiEventEnabled`。
...
> （全有或全無，不會混在一起）。這裡空著的時候才退回 `assets/{slug}/cards/`。

> **沒有內建範例卡**：兩邊都沒有圖的時候，抽卡頁的按鈕是**停用**的，
> 底下寫一行「等待新人上傳照片」。以前這裡會退回一組寫死在 `js/draw.js`
> 的生活照，新開的站台等於讓賓客抽到別人的照片。

#### 賓客的收藏可以放大看、可以存下來

抽卡頁下面「你的收藏」裡的小卡只有指甲大，看不清楚照片。
**點一下**（鍵盤 Enter 也可以）會蓋上一張大卡。

大卡上**只有照片**，`SSR`／`SR` 多一層會動的彩虹光膜 ——
等級、卡名、說明都不疊上去，放大看的重點就是那張照片。
（那些字在小卡上停留就看得到。）

大卡下面有「**儲存下載**」，存成一張 **JPG**：照片，底下一行**新人名字・日期**，
其餘什麼都不加。按 Esc、點卡片以外的地方、或右上角的 ✕ 都關得掉。
存去哪裡和感謝信同一套：**手機進相簿**（系統分享單，或長按存圖），桌機才是下載檔案。

> 卡圖如果指到**外部網址**（不是後台上傳、也不在素材資料夾裡），
> 瀏覽器不一定讓網頁把它畫成圖檔。這種情況會提示改用長按（電腦按右鍵）另存。
...
清單上兩種**長得不一樣**：章節是一條有底色、左邊帶色帶的橫幅，
故事往內縮一格掛在它底下 —— 不用讀標籤就看得出來分段在哪裡。

順序用左邊的 **⠿ 直接拖曳**（和測驗題目同一套），
或按每一列的 **⋯** 上移／下移／移到最前／移到最後。
放開之後會把整份重新編成 1…n，改錯了 toast 上有「復原」。
...
### 8. 收禮小幫手（`/butler` ＋ 後台「收禮小幫手」分頁）

> 這是**要另外打開**的功能：`npm run set-pages -- --slug {slug} --enable butler`

婚宴當天在收禮台收禮金、發禮餅用的工具。
它是**另一個網址**、和站台網址無關：

```
https://{網域}/butler#{一串代號}
```

幫忙收禮的親友（通常四五位）點連結、輸入一組**六位數通行碼**就能用，
不必辦 Google 帳號、不必進後台、也看不到出席回覆與悄悄話。

#### 現場長怎樣

畫面和**新人後台是同一套**（同樣的字族、色票、1px 線條與元件），
所以新人看得懂、小幫手也不必重新學。上面三個分頁：

| 分頁 | 做什麼 |
|---|---|
| **收禮台** | 賓客名單。搜尋名字／編號／桌次，點一下就開記帳表單。已收的那幾位帶著「已收」標記、右邊直接寫著收了多少 |
| **紀錄** | 所有人記過的每一筆，新的在最前面。點進去可以改、可以刪 |
| **統計** | 禮金總額、禮餅總盒數、發出禮餅筆數、到場人數、還有幾位沒收到，以及**誰記了多少** |

記帳表單問的就是那幾件事：**禮金金額**（一排常見金額點一下就填好：1200、1600、
2000、2200、2600、3600、6000、12000）、**禮餅發了沒**、**共幾盒**、**共幾人**、**備註**。
盒數與人數是加減按鈕，不用叫鍵盤出來。

名單上沒有的賓客按「**＋ 名單外**」直接記，姓名自己打就好。

每個人第一次用會被問一次「你叫什麼名字」（存在自己手機裡），
之後每一筆都會記下是誰收的 —— 事後對帳時，「誰手上該有多少現金」一看就知道。

> **幾個人同時收也不會打架**：每一筆收禮是資料庫裡自己一份文件，
> 不是塞在同一份裡，所以四五個人同時按儲存不會互相蓋掉。
> 大家的畫面與後台都是即時訂閱：阿華記的那一筆，
> 小真的手機和新人的後台**當下**就看得到，不必等誰把資料交回來。

> **會場網路很爛也沒關係**：Firestore 會把寫入排進手機自己的佇列，
> 斷線時照樣按得下「儲存」，恢復連線自己送出去。
> 頂列那顆小圓點只是告知連線狀態，不會擋住任何操作。
...
| 方向 | 內容 |
|---|---|
| 後台 → butler | **匯入賓客名單**（一份快照） |
| butler → 後台 | 現場記下的**每一筆與統計**，即時出現在後台 |

後台這一側分成三個子分頁：

| 子分頁 | 做什麼 |
|---|---|
| **收禮統計** | 禮金總額、禮餅總盒數、發出禮餅、到場人數、名單未收，以及**誰記了多少** |
| **收禮明細** | 現場記下來的每一筆（桌機是表格、手機是一筆一張卡），可以搜尋、匯出 CSV |
| **連結與名單** | 產生／停用／刪除連結，把賓客名單匯過去 |

> **一組連結都還沒產生時，這個分頁其實什麼都做不了**：統計是 0、明細是空的、
> 名單也匯不出去。所以那時候點「收禮小幫手」會**直接落在「連結與名單」**，
> 而不是一張全是 0 的統計；另外兩個子分頁最上面橫一條提醒，
> 上面就有「產生第一組連結」。（「連結與名單」那一頁本來就有一張完整的空狀態卡，
> 不重複再掛一條。）

> **金額預設是遮起來的**（像網路銀行那樣顯示 `$ ---`），按「顯示金額」才會露出來。
> 婚宴當天這一頁常常就開著擺在收禮台上、或拿在手上給家人看，
> 旁邊經過的人不該一眼看到今天收了多少。
> 這個開關**不會被記住**，每次重新打開後台都回到遮住的狀態；
> 匯出的 CSV 不受影響（要匯出就是要看數字）。

到後台「**收禮小幫手 → 連結與名單**」按「產生新連結」，會拿到一組網址加一組通行碼，
兩個一起傳給幫忙的親友（有一顆「複製連結＋通行碼」直接複製成一則訊息）。

> **通常一場婚禮只要一組**。四五個人共用同一組，統計才會加在一起。
> 真的要分（收禮台一組、送客桌一組）也可以，後台會把所有連結的數字加總。

名單從哪裡來，匯入時自己選：

| 來源 | 帶得到什麼 | 什麼時候用 |
|---|---|---|
| **排桌名單** | 編號（A01）、姓名、桌次、人數、類別、備註 | 有開排桌管理時最推薦，收禮台核對最快 |
| **出席回覆** | 姓名、人數、與新人的關係、備註（桌次會盡量從桌次名單比對補上） | 沒開排桌管理時用 |

> **名單是「匯過去的一份快照」，不是接上去的即時資料。**
> 婚禮當天收禮台要的是「現在這一版」，不該因為新人在休息室回頭調了一下桌次
> 就跟著跳。名單改了就再按一次匯入（整份換掉），**已經記好的收禮不受影響**。
> 說了不來的人不會被匯進去。上限 600 位。

「**停用**」會讓拿著連結的人看到「已停用」、也記不進新的資料，
但紀錄與統計都留著，隨時可以重新啟用 —— 婚禮結束後按一下就好。
「**刪除**」則是連紀錄一起刪掉，要打字確認。

#### 通行碼是真的有用，不是畫面上的一道門

一般的「密碼框」在這套架構下是假的：Firestore 的讀取請求不帶 payload，
Security Rules 沒有辦法驗證使用者輸入的密碼（見下面〈安全性設計〉）。

所以這裡換了個作法 —— **讓通行碼算得出資料在哪裡**：

```
收禮簿的位置 = PBKDF2-SHA256(通行碼, salt = 網址上的代號, 20 萬次)
```

| 拿到的東西 | 結果 |
|---|---|
| 只有連結 | 少了通行碼，算不出位置，讀不到任何東西 |
| 只有通行碼 | 少了連結上的代號，同樣算不出來 |
| 兩個都有 | 才走得到那份資料 |

規則那邊只做一件對應的事：**單筆讀取全開、列出整個集合一律拒絕** ——
能列出集合的話，上面那道門就整個白做了。
20 萬次迭代不是為了防連結被猜到（那串代號有 110 bits，猜不到），
是為了讓「拿到連結之後再慢慢試通行碼」變得不划算。

> **所以連結與通行碼要當成鑰匙看待**：兩個都外流，禮金資料就外流了。
> 傳給幫忙的親友就好，不要貼在群組裡；婚禮結束後順手按「停用」。
> 真正需要綁身分才讀得到的資料（出席回覆、悄悄話）一律不在這裡。

同一支手機開過一次就記住，不必再輸入通行碼 ——
婚宴當天不會有人想輸入第二次。
...
> ⚠️ **同一個名字只留一個副檔名。**
> `cover.jpg` 跟 `cover.jpeg` 同時存在時只會用到 `.jpg`（依上表的順序），
> 換圖換到另外那一個就會「怎麼傳都沒變」。`sync-assets` 會把這種重複
> 單獨列出來（跟「檔名沒對上」分開講），照它說的把多的刪掉就好。
...
> 大廳永遠會產（它是最常被分享的那一個網址）；
> 邀請函則跟著 `pages.rsvp` 開關走，關掉的站台不產。

### 要幫哪些站台產？

站台清單是**素材資料夾 ∪ Firestore 的 `slugs` 集合**：

- 兩邊都有 → 用照片合成
- 只有 Firestore（新人還沒給照片）→ 照樣產 HTML，掛共用的 `og-default.jpg`
- 只有資料夾、Firestore 裡沒有 → 會產，但**印出警告**：
  資料夾名稱必須跟 slug 一模一樣，否則賓客走的網址不會命中產出來的檔案

> 沒設 Admin 憑證時讀不到 Firestore，清單就只剩素材資料夾 ——
> 沒放照片的站台會整個被漏掉，連結仍然停在工作室 logo。
> 這是另一個「跑 build-og 前先設好憑證」的理由。
...
| 都沒有（含沒有素材資料夾的站台） | 指向全站共用的 `/og-default.jpg`（品牌底色 ＋ 置中 logo） |
...
用 iPhone 尺寸的 Chromium 跑完整場：

```
[1] 通行碼        # 錯的進不去、對的進得去、網址少了代號直接擋下來
[2] 記一筆禮金    # 搜尋 → 金額捷徑 → 禮餅盒數 → 儲存 → 真的寫進 Firestore
[3] 統計          # 不同小幫手記的加在一起，看得出誰記了多少
[4] 名單外的賓客  # 自己填名字，不會掛到名單上任何一位身上
[5] 停用          # 新人收回連結之後就進不去了
```

> 測試裡的 `deriveBookId()` 用 Node 的 `pbkdf2Sync` 重算一次收禮簿的位置，
> 和瀏覽器端 `js/butler-key.js` 的 `derive()` **必須算出同一個值** ——
> 那一行就是兩邊的契約，改了參數（迭代次數、salt 格式）兩邊要一起改，
> 不然「通行碼對了也進不去」。
...
```
[1]  /w/{slug}/invitation   # 內容、時區、倒數、hashtag，以及「那一頁只剩表單」
                            # 以及「與其他頁共用版型」：導覽列、浮動控制、common.css
[1a] 出席回覆的題目         # 三個出席選項、關係／聯絡方式／喜帖／喜餅、
                            # 條件欄位、葷素連動、兩個「同上」捷徑
[2]  /w/wu-yang-1220        # 另一組 slug，內容互不干擾
[3]  /w/does-not-exist      # 中文找不到畫面，非白畫面且無 console 錯誤
[4]  draft 站台             # 未發布顯示找不到畫面
[4b] RSVP 截止與關閉
[5]  RSVP 送出流程          # 不跳頁、成功狀態、寫入欄位正確、結果卡的兩個出口
[5b] 新人關掉郵寄           # 喜帖與喜餅都不出現郵寄那個選項
...
抽屜          # .ad-drawer 的規格（尺寸、層級、dialog 語意、
              # 遮罩、✕ 的字級、CTA 貼底）
...
| `sites/{siteId}/butlerLinks` | 只有新人 | 只有新人 | 只有新人 | 只有新人 |
| `butlers/{bookId}` | ✅ 單筆讀取・**不准列出集合** | 只有新人 | 只有新人 | 只有新人 |
| `butlers/{bookId}/entries` | ✅ | 拿著連結與通行碼的小幫手 | 同左 | 同左／新人 |
| `slugs/{slug}` | ✅ | ❌ | ❌ | ❌ |
| `short/{code}` | ✅ | ❌ | ❌ | ❌ |
...
> `butlers/{bookId}` 那兩列是這份表裡唯一不靠 Auth 身分的保護：
> 文件的位置本身是「連結上的代號 ＋ 通行碼」推導出來的，
> 兩樣都有才走得到（見〈收禮小幫手〉）。所以那裡**只放收禮台當下需要的欄位**，
> 需要綁身分才讀得到的資料（出席回覆、悄悄話、排桌草稿）一律不進去。
> `allow list: if false` 是這套設計唯一的單點 —— 能列出集合就等於門沒關。
