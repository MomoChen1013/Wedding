...
- **首頁**：固定背景（一張圖或一段影片，滾動時不動）→ 置中開場（`h1` + `.cn`）→
  ...
  RSVP → 卡片連結（兩欄，內建七張＋新人在後台自訂的卡片＋最後一張照片集）。
...
- **出席回覆**（`/w/{slug}/invitation`）走的是同一套版型 ——
  ...
  **那一頁只做一件事：收下回覆** —— 婚禮資訊、兩人的故事、照片集都在首頁，
...
sites/{siteId}
  ...
  rsvpShowGallery(bool)         # 首頁 Explore 要不要放「照片集」
...
> **那一頁只剩出席回覆**：地點、交通、Dress Code、禮金、兩人的故事與照片集
> 都只出現在首頁，出席表單不再放第二次 —— 會拿到那個連結的人是來回覆的，
> 那幾塊擺在表單前面只會把要填的東西往下推。
> 賓客送出之後，結果卡上有「**回婚禮大廳**」與「**看我們的故事**」兩個出口。
> 照片集的開關搬到「婚禮資訊 → 自訂內容」那一頁（它現在是首頁 Explore 的
> 最後一張卡）；照片本身放在素材資料夾（見〈素材資料夾〉），後台不能自己上傳。
...
**照片集是最後一張卡**：它本來在邀請函上，邀請函現在只剩出席回覆的表單，
所以照片集搬到這裡 —— 它本來就比較接近「逛一逛」。
排在自訂卡片**後面**（那是「看完了再翻翻照片」，不是要賓客先去做的事），
點了原地跳出兩欄的照片牆。開關在同一頁最上面的「照片集」那一區，
**按下去就存**；照片本身放在素材資料夾，後台不能自己上傳。
