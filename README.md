...
| 開始製作 | 有順序的幾步，每一步右邊一顆直達那一頁的按鈕。**已經填好的自動打勾**（判斷是拿資料本身算的，不另外記進度 —— 換一台裝置也不會歸零），沒開的功能不會出現在清單裡 |
| 給你的製作筆記 | 一張收起來的卡（`<details>`），只露出標題與副標題。展開是**建議的節奏**：拿到後台的第一週／有空的時候慢慢做／婚禮前一到兩週／婚禮前一天／結束後 —— 上面那幾步講「做什麼」，這一段講「什麼時候做」 |
