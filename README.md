...
畫面最下面有一條**固定的底列**：左邊寫「還有 N 位沒排」（點下去回到未安排區），
中間是**儲存排桌**，右邊的 **⋮** 收著送出、復原、重做與四個統計。
...
|---|---|
| 還沒送出 | 還沒送到賓客的查座位頁 |
| 已送出 | 最後送出：2026/08/18 20:30 |
| 有修改還沒送出 | 排桌改過了，還沒重新送出（按鈕變成「再送一次」） |

> 這幾句刻意不用「同步」兩個字：新人不見得知道「桌次查詢」是哪一個畫面，
> 但「賓客的查座位頁」一定看得懂。
