...
> 「出席回覆總覽」「回覆資訊」「設定賓客標籤」）；首頁卡片放在婚禮資訊的「自訂內容」子分頁。
>
...
- **長表單的「儲存」會浮在畫面最下面**（婚禮資訊、當日流程、表單設定），
  有還沒存的變更時右邊會寫「有還沒儲存的變更」，切到別的分頁也會提醒一聲。
...
> 照片的開關搬到「婚禮資訊 → 自訂內容」那一頁（它現在是首頁上自己的一段，
...
開關在「婚禮資訊 → 自訂內容」最上面的「相遇之間」那一區，**按下去就存**
...
| Dress Code | 三件事：**說明文字**、**最多 4 個顏色**、**最多 5 張參考圖**（見下面） |
| 關於禮金 | 同上 |
| 兩人的故事 | 大廳的 Our Story 區塊；**留白則大廳不出現**（出席回覆那一頁不再放第二次） |
...
| 在後台 | 賓客那一頁 |
|---|---|
| **顏色**：一排色塊，點一個就跳出系統原生的顏色選擇器。還沒設定過時預設攤開 **3 格白色（#FFFFFF）**，最後面一顆「**＋**」，點了直接選色補上第四個；補滿 4 個之後「＋」就不再出現。每一個色塊右上角有 ✕ 可以移除 | 大廳的「出席前的小提醒」多一排**實心的不規則圓**，就是那幾個顏色（四顆各用一組不同的圓角，排在一起不像複製貼上） |
| **參考圖**：最多 5 張，可以用 ←／→ 排順序、單張刪除。第一張標著「預設顯示」 | 卡片上**只露第一張**，其餘要點「**查看更多**」才在彈窗裡看完整的一疊 |

兩者都是**選好就存**，不必按下面的「儲存婚禮資訊」（和旁邊的交通圖片同一個規矩，欄位旁邊也寫了）。
三樣（文字、顏色、參考圖）**任何一樣有設定，大廳就會出現這一塊**；三樣都留白才整塊收起來。
