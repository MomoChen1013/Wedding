...
| `public/js/invitation.js` | 全檔 | 封面／倒數／hashtag，最後 `RSVPForm.mount()`。**婚禮資訊、兩人的故事、「相遇之間」、`.ics` 都已經回到大廳**，那一頁只剩表單 |
...
### Phase 3 — Admin：活動編輯器 ✅ 已完成

- `admin.html`：lobby 分頁加「婚禮流程」子分頁（`hidden`，`multiEventOn()` 才出現）
- `admin.js`：活動卡列表、型別預設、↑↓ 排序、dirty 追蹤、刪除前的影響提示
- 婚禮資訊分頁：多活動時場地三欄轉唯讀 ＋ 說明 ＋ 跳轉鈕
- 表單設定分頁：多活動時上方長出「每個活動各自要問的」（見 6.3），
  per-event 開關寫進 `events[].askCount/askMeal/askChildSeat/askDiet`；
  共用題目的既有欄位與畫面**一個都不動**
- `admin.css`：`.ad-ev*` / `.ad-evask*`，全部用既有 token，↑↓ 併進既有的共用選擇器
- **驗收**：`tests/multipage.mjs` 新增 `[14d] 後台婚禮流程`，41 條斷言。
  五個測試套件全綠。
...
| 決定 | 理由 |
|---|---|
| **場地鏡像回站台文件** | 存好之後把主要活動的地點寫回 `venueName`／`venueAddress`／`venueMapUrl`。分享縮圖（`build-og`）與大廳的既有欄位讀的還是那一份，不鏡像回去兩邊會對不起來。婚禮資訊那三欄因此轉成唯讀 |
| **不碰 `eventDate`** | 主日期一改，倒數計時與分享縮圖都會跟著跑，維持「要透過我們才能改」的既有規矩。主要活動的日期和主日期不一樣時**只出一句提醒**，不自動改 |
| **一次只展開一張卡** | 五個活動全部攤開的話這一頁會長到滑不完 |
| **id 產生後永不改變** | `ev_<type>_<4 碼隨機>`。帶型別只是為了在 Firestore 裡看得懂，隨機碼才是唯一性來源；不重用刪掉的 id，免得舊回覆對到新活動 |
| **「每個活動各自要問的」只在 ≥2 個要回覆的活動時出現** | 只有一場的話，多一個標題只是把同樣的題目搬個位置。這是「簡單婚禮不能被複雜化」在後台的具體長相 |
| **per-event 開關是即存即生效** | 只改 `events[]` 裡那一個活動的那一個布林，其餘欄位原封不動帶回去 —— 新人可能剛在「婚禮流程」改過，用畫面快照會把那邊的改動蓋掉 |

**刻意沒做「從當日流程匯入」**：報告原本列了這個便利功能，實作時判斷它
**期望值是負的** —— `schedule[]` 裡多半是婚宴內部的流程（「入場迎賓」「送客」），
一鍵匯入會產生一堆垃圾活動，新人還要一個一個刪，比沒有這顆按鈕更糟。
真的有新人需要再說。

**踩到的一個坑**：「每個活動各自要問的」那一組收起來時，原本只設 `hidden`，
但每個活動都帶一條「能來參加嗎」的固定題目勾選框 ——
既有測試「固定題目有幾條」數到了看不見的那些（8 變成 9）。
收起來的時候要**連 DOM 一起清掉**。看不見的東西不該還算數。
...
- **驗收**：`tests/multipage.mjs` 新增 `[6d] 多活動的出席回覆`，49 條斷言。
  五個測試套件全綠。

**實作時改掉的一個設計**：活動卡答完就收合，原本寫成「答完即收」——
實際跑起來才發現**說了「會參加」的那一張一收起來，賓客就填不到人數與葷素了**。
改成**手風琴**：剛回答的那一張留著開，回答下一張時前一張才收；
說「無法參加」的沒有東西要填，可以馬上收。
（`tests/multipage.mjs` 有一條測試釘住「剛回答的那張要留著開」。）

**踩到的兩個坑**：

1. **預先產好的大廳是舊的**。`public/w/{slug}/index.html` 是 `build-og` 的產物、
   而且進版控，Hosting 的靜態檔優先於 rewrite —— 改了骨架不重跑 `build-og`，
   賓客看到的還是舊版面。這一次直接把同樣的兩行 id 補進那份產物，
   **正式部署前仍然要跑一次 `npm run build-og`**。
2. **共用測試站台的名字會撞**。新測試送出的回覆用了「王小明」，
   把標籤那一段的固定資料弄髒了（「沒有標籤」的篩選、彈窗裡關不掉的那個標籤
   都跟著壞）。測試站台是共用的，新資料要用不會撞的名字。

### Phase 4b — Event 追加題目 ✅ 已完成

- 後台：活動卡底下的「追加題目」，型態限死單選（≤4 選項）／文字，每個活動 ≤3 題
- 前台：單選畫成 `.choice.sm`（和「與新人的關係」同一個元件），文字是一般輸入框；
  作答存進 `events[eventId].answers`
- CSV：單選存 id、匯出時換回文字
- **選項存 id 不存文字**：新人日後改選項的字，已送出的作答還對得回來
  （和 `guestTags` 同一個理由）。id 只在第一次有文字時定下來，之後改字不動 id

### Phase 5 — Admin 統計與名單 ✅ 已完成

- 各活動出席統計表（總覽頁上方，只有多活動時才出現）
- 名單多出每個活動一欄：`✓ 4`／`✗`／`—`
  （**`—` 是「還沒回答」，和 `✗` 是兩件事，不能混成一個符號**）
- 詳細抽屜多一段「各活動出席」，含素食人數、備註與追加題目的作答
- CSV 動態欄位，`admin.js` 與 `scripts/export-rsvps.js` 兩邊同步；
  單一活動時 CSV 欄位與改版前**逐字相同**
- **驗收**：三個活動三個獨立分母、三個桶子加起來等於總回覆筆數、
  舊資料落在「待回覆」而不是「不出席」
