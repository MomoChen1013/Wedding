...
info-card
  ...
  日期   2026.10.18・星期六          ← 全部同一天才出現

  ┌ info-ev-list ─────────────────────
  │ CEREMONY                          ← .inv-kicker 語彙（11px/.34em/大寫）
  │ 證婚                               ← 15px --ink
  │ 10/18 SAT 14:00                   ← 13px tabular-nums --ink-soft
  │ 台北真理堂                          ← .ir-val
...
1. **不用 `.info-row` 的兩欄 grid**（`4.5em 1fr`）。左欄放得下「證婚」「婚宴」，
   放不下「婚後派對」「基督教證婚」這種 4–5 字，更放不下英文 kicker。
   改成堆疊段落，名稱多長都不會擠壞。
2. **英文 kicker ＋ 中文名**的組合和邀請函的活動卡**同一套語彙、不同密度** ——
   大廳只讀資訊（沒有按鈕），邀請函要回覆（有 會參加／無法參加）。
   一致但不重複。
3. **「開啟地圖」下放到每個活動**，全域那顆移除。
   一個活動一個地圖連結，這是第六節「不限制地址數量」在大廳的具體長相。
4. **`requiresRsvp:false` 的活動照樣列出來**（文訂、迎娶）——
   大廳講的是「這場婚禮有哪些事」，不是「你要回覆什麼」。
   要回覆什麼是邀請函的事。

新增的 class 只有 `.info-ev-list` / `.info-ev` / `.info-ev-kicker` / `.info-ev-name`
/ `.info-ev-time`，放進既有的 `css/index.css`，全部用既有 token。
三份骨架各加一個 `<div class="info-ev-list" id="infoEvents" hidden></div>`。

#### 5.7.3 「加入行事曆」怎麼處理三個活動

目前**大廳與邀請函是兩套實作**，而且大廳那一套支援不了多活動：

| | 現在的做法 | 支援多活動？ |
|---|---|---|
| 大廳 | `calendar.google.com/render?...` 網址（`js/index.js` L277–299） | ❌ 一個網址只能一個事件 |
| 邀請函 | 前端產生 `.ics` 下載（`js/invitation.js` `setupCalendar()`） | ✅ 一個檔可以放多個 `VEVENT` |

提案：**多活動時，大廳也改成產生 `.ics`**，一次把全部活動加進去
（`.ics` 規格本來就允許一個 `VCALENDAR` 裡放多個 `VEVENT`，
`setupCalendar()` 只要把單一事件的那段 map 成陣列）。
單一活動時大廳維持現在的 Google Calendar 網址，**不動**。

順手把兩套實作收斂成 `common.js` 的一個 `buildIcs(events)`，
大廳與邀請函共用 —— 現在是同一件事寫了兩次。

#### 5.7.4 倒數計時要對哪一個活動

`startCountdown(cdGrid, W.dateISO)` 目前對 `eventDate` 倒數。

**維持對 `eventDate`（＝主要活動／婚宴）**，理由是可預期：
數字不會因為文訂過了就突然跳一大格。
多活動時只改 `#cdTarget` 那行文字，標明是哪一場：
...
（「倒數對最早尚未發生的活動」對賓客其實更有用，但數字會在每個活動結束後
跳一次，解釋成本高。列為日後可再議，v1 不做。）

#### 5.7.5 交通資訊（Access）——**這是唯一一個要老實承認的限制**
...
| 選項 | 評估 |
|---|---|
| (a) 下放到每個活動 `events[].transportPublic/Parking` ＋ 圖 | ❌ **否決**。圖片是 data URL（每張 ≤150KB），10 個活動 × 2 張 ≈ 3MB，**會爆 Firestore 單一文件 1MB 上限** |
| (b) **維持全站一組**，語意明確定義為「主要活動的交通」 | ✅ **推薦**。實務上真正需要詳細交通說明的就是婚宴那一場；證婚在教堂、After Party 在酒吧，一個地圖連結就夠 |
| (c) 每個活動一段純文字交通說明（`events[].transportNote` ≤200，無圖） | 折衷，可延後。真的有新人反映再加 |

採 (b)。多活動時在 Access 區塊標題下加一行說明，
不要讓賓客以為那是所有場地共用的：
...
`#infoVenueJump`（「看交通資訊 →」）多活動時只掛在主要活動那一段上。
...
| 檔案 | 改什麼 |
|---|---|
| `public/index.html` | 加 `#infoEvents` 容器 |
| `public/lobby-korean.html` | 同上 |
| `public/lobby-forest.html` | 同上 |
| `public/js/index.js` L260–300 | 活動列表渲染、地圖連結下放、`.ics`、倒數文字、Access 標題 |
| `public/css/index.css` | `.info-ev-list` / `.info-ev` 等，全部用既有 token |
| `public/js/common.js` | 新增共用的 `buildIcs(events)`（大廳與邀請函收斂成一份） |
| `public/js/invitation.js` | `setupCalendar()` 改用 `buildIcs()` |
...
- **大廳**（見 5.7）：三份骨架加 `#infoEvents` 容器；`index.js` 的活動列表、
  地圖連結下放、Access 標題、倒數文字；`common.js` 抽出共用的 `buildIcs(events)`，
  大廳與邀請函收斂成一份
...
10. **交通資訊維持全站一組**，語意定義為「主要活動的交通」，
    區塊標題加註是哪個場地（5.7.5 的選項 b）—— 接受嗎？
    這是唯一一個真的做不到 per-event 的東西（圖片是 data URL，會爆文件上限）。
11. **倒數計時固定對主要活動**，只在文字上標明是哪一場（5.7.4）—— 接受嗎？
    （另一個選項是「對最早尚未發生的活動」，數字會在每個活動結束後跳一次）
12. **當日流程（Schedule）與活動刻意不合併**（5.7.6）—— 接受嗎？
    合併需要 `schedule[].eventId`，而且新人會搞不清楚「入場迎賓」該寫在哪。
