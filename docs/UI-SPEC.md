...
| 圓角克制 | 後台 `--radius: 4px`（賓客頁 `2px`）。**膠囊 `999px` 只給「可以選起來的東西」**（chip、pill 文字按鈕）；唯讀標記與按鈕都是方角 |
...
> **膠囊只給「可以選起來的東西」。** 改版前 tag 和 chip、Badge、pill 按鈕
> 全都是 999px 膠囊，唯一的區別是「可點的有 hover 反應」——
> 那是一個要伸手去試才知道的區別。現在是三個家族：
>
> | 家族 | 形狀 | 怎麼認 | 例子 |
> |---|---|---|---|
> | **唯讀標記** | 方角 `--radius` | 細框、11px、沒有任何 hover 反應 | `.ad-tag`、`.ad-badge` |
> | **開關** | **膠囊 `999px`** | 有 on／off，或按一下切換一個條件 | `.ad-chip`、`.ad-filtersum-clear` |
> | **動作** | 方角 `--radius` | 實心底或 ghost 框、13–14px、按下去會發生一件事 | `.btn`、`.btn.small` |
>
> 所以：**一顆按鈕不會長成膠囊。**「＋ 新增賓客」是動作不是條件，
> 做成膠囊的話讀起來像「兩個可以勾的選項」。
>
> 「點不動的 chip」也不存在：要嘛它是 Disabled（暫時選不到，見 3.4），
...
| `.ad-textarea` | 多一層 `--bg2` 的面 ＋ `padding:10px 12px`，底線留著。它的 `::placeholder` 是 **`--ink-soft`** 不是 `--ink-3` —— `--ink-3` 站在 `--bg2` 上只有 4.07:1（§2.1）。單行欄位是透明底、站在 `--bg1` 上，維持 `--ink-3` |
...
| 05 的提醒切換 | 同一支切換列（`#adWzGuideSwitch`，「共 N 項提醒・第 i / N 項」）。勾了四項就是四段 textarea 疊在一起、一頁捲不完，而每一段的小標長得一樣 —— 捲到中間分不出自己在填第幾段。**只勾一項時切換列收起來**，那一項直接顯示。剛勾起來的那一項會直接跳過去（他勾它就是要填它）；收起正在看的那一項時停在同一個位置，不跳回第一項 |
