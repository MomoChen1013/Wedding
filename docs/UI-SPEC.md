...
| 每一頁先問一句 | `.ad-ask`（3.15a）：用人的角度問「現在要完成什麼」 |
...
**排桌工作區是「所有數字都用 `--primary-deep`」的例外。** 那一頁一個螢幕上
同時有幾十張桌卡，每一張都有桌號、人數、剩餘位子、標籤；數字全部上金色的話，
...
| `--seat-over`（超量：桌卡外框與「超過容量 N 位」） | `#E86C93` —— **整頁唯一要被看到的狀態** |
...
### 3.15a 問句 `.ad-ask` ＋ 細節 `.ad-fine`

**每一頁都要先用人話問一句「現在要完成什麼」。**

「婚禮資訊」的六個階段本來就是這樣做的（`.ad-wz-title`）。其他分頁原本沒有那一句：
標題是一個名詞（「自訂內容」「感謝信」），接著就是一段三四行的說明 ——
新人得先讀完，才知道自己在這一頁要做什麼。

```html
<h2 class="ad-sec-title ad-page-title">…感謝信</h2>
<p class="ad-ask">想對誰說聲謝謝？</p>
<p class="ad-sec-note">賓客在婚禮那天輸入自己的名字，就會領到你們寫給他的那一封。</p>
<details class="ad-fine">
  <summary>兩種信差在哪裡</summary>
  <div class="ad-fine-body">…</div>
</details>
```

字級刻意排在標題與說明之間，四階各自只回答一個問題：

| Class | 字級 | 回答的問題 |
|---|---|---|
| `.ad-page-title` | 21px 明朝體 | 這一頁叫什麼（和側欄那顆分頁同名） |
| `.ad-ask` | 17px 明朝體 | **這一頁在問你什麼** |
| `.ad-page-sub` | 12px sans | 現在的狀態（幾筆、幾位，由 JS 填） |
| `.ad-sec-note` | 12.5px sans | 真的能降低理解成本的那一行 |

規則：

- 問句**用問句**（「想問賓客哪些事？」），不要用動作名稱（「設定出席表單」）。
- `.ad-sec-note` 最多**一行**。講不完的收進 `.ad-fine`，不要讓它長回三行。
- `.ad-fine` 是純 `<details>`（和首頁的 `.ad-note`、常見問題同一套），**不寫 JS**。
  `summary` 是一句名詞短語（「收起來之後會發生什麼事」），不是「更多」。
- 工作台（排桌工作區）用 `.ad-ask-tight`：那一頁的高度要留給版面本身。
