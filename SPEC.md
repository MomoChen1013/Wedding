...
sites/{siteId}
  ...
  seatingSearchEnabled : boolean  # 桌次頁的搜尋開關（後台「桌次 → 桌次搜尋及名單」
                                  # 那顆 switch），沒有這個欄位視為 true；
  ...
  rsvpShowGallery : boolean  # 首頁要不要放「Moments 相遇之間」（本來在邀請函上）
...
> 這幾題**只在側欄的「出席表單設定」設**（活動卡上的「本場次專屬問題」，
> 新增／編輯走 Modal、排序走拖曳）。「婚禮資訊 → 婚禮流程」那一段只管
...
由新人在後台「婚禮資訊」頁上的「婚禮流程」那一段自己維護
（一個活動一張卡：順序用拖的、內容點「編輯」在 Modal 裡改，改完就存）。
...
「相遇之間」在大廳都有，在這裡再放一次，等於把真正要做的那件事一路往下推。
...
- 照片：搬到大廳自己的一段「Moments 相遇之間」（見 13.3.1），
...
| ★ 更具體是哪一種？ | 單選、選填；選項＝新人開放的賓客標籤 | 開了標籤功能、而且至少有一個標籤放在「賓客表單自選標籤」那一組時 |
...
（前台與 `check-site.js` 都還讀那一欄），所以 `pagePublish.seating`
只接手排程時間，開／關仍然讀那個舊欄位 —— 兩個地方不會各記一份
然後對不起來。`savePagePublish()` 寫的時候兩欄一起更新。
...
**不是每個開關都在 `pages` 裡**：`guestTagsEnabled`（設定賓客標籤）與
`multiEventEnabled`（多活動）是站台文件上自己的欄位，新人改不動。

- **設定賓客標籤**是側欄自己的一頁，走的是同一套三狀態 ——
  判斷直接寫在 `tabState()` 裡（開著是 `on`、沒開是 `locked`，可加購），
  不查 `UNRELEASED_FEATURES`。鎖起來的樣子和其他分頁一模一樣。
- **多活動**不是一頁，是「婚禮資訊 → 婚禮流程」那一段裡的活動卡：
  沒開的時候整段收起來（`#adEvBox`），時間與地點就是那幾個欄位；
  開了之後那幾欄變成主要活動的鏡像（收起來），改在活動卡上。
...
| 頁面設定 | 永遠都在（後台自己的一頁；一頁都沒得收的站台會整顆收起來） |
| 常見問題 | 永遠都在（後台自己的一頁） |
| 出席表單設定 | `rsvp`（和出席回覆同一個開關 —— 它就是那一頁的題目） |
| 設定賓客標籤 | `guestTagsEnabled`（站台文件上的欄位，不在 `pages` 裡） |
| 出席回覆 | `rsvp` |
| 婚禮資訊 | 永遠都在（大廳是必開的頁面） |
| 自訂內容 | 永遠都在（Explore 區屬於大廳） |
| 桌次圖 | `seating`（只看 `pages`：新人自己把「開放桌次功能」關起來時，後台照樣進得去） |
| 感謝信 | `letter` |
| 婚禮小卡 | `draw` |
| 新人故事牆 | `exhibition` |
| 新人測驗 | `quiz` |
...
1. 開場：網站已經做好了，接下來要放什麼內容都在這裡改。右上角兩個出口：
   **查看婚禮網站**（賓客現在打開連結看到的樣子）與**查看表單**。
2. **給你的製作筆記**（`.ad-note`）：一張 `<details>`，預設收著、只露出標題與
   副標題，展開是建議的節奏（第一週／慢慢做／前一到兩週／前一天／結束後）。
   下面第 3 點講「做什麼」，這一段講「什麼時候做」，所以排在步驟前面 ——
   第一次進來的人最先要的是「我現在該不該急」。收著是因為
   一整份時程攤開在首頁比十顆分頁名稱更嚇人。
3. **開始製作**（`HOME_STEPS`）：有順序的幾步，每一步右邊一顆
   `data-empty-hash` 的按鈕直達那一頁；沒開的功能整步不出現
   （叫新人去點一顆鎖著的分頁是最糟的第一步）。完成訊息（`.ad-step-state`）
   接在標題後面而不是自己一行 —— 一列省一行，窄螢幕上它跟著標題折行。
   「婚禮前有空可以慢慢做」那一步涵蓋感謝信／測驗／故事牆／婚禮小卡四個功能
   （`HOME_EXTRAS`），沒開通的不算在分母裡，按鈕指向第一個還空著的那一頁。
   第一步「**把後台放到手機主畫面**」是唯一不跳分頁的：操作步驟就地展開，
   按「我已經完成」才收起來，打勾記在 localStorage（`HOME_PIN_KEY`）——
   它問的是「這一台裝置做過了沒」，資料庫裡沒有、也不該有那個答案。
   它的按鈕也不在右邊那一欄（那一欄垂直置中，展開時會掉到清單中間），
   而是跟著標題走，就在標題右邊（`.ad-step-head`）。
4. 最後兩個出口（`.ad-more`）：**頁面設定**與**常見問題**。
   兩者原本是首頁最下面的兩段，現在各自是側欄的一顆分頁（見下面兩段），
   首頁只留卡片指過去 —— 第一次照著走完的人，下一件事通常就是
   「決定現在讓賓客看到哪幾頁」。

**側欄的結構**：最上面是**首頁**與**頁面設定**（`.ad-navtop`），
最下面是**常見問題**（`.ad-navbot`）—— 三者都不是「在編輯某一塊賓客內容」，
所以不進任何分組。中間是三個可摺疊的分組（`.ad-navgroup`）：

| 分組 | 分頁 |
|---|---|
| 婚禮內容 | 婚禮資訊、自訂內容、出席表單設定、設定賓客標籤 |
| 婚宴管理 | 出席回覆、桌次圖、排桌管理（`.is-sub`）、收禮小幫手 |
| 賓客互動 | 感謝信、婚禮小卡、新人故事牆、悄悄話、新人測驗 |

`TAB_PAGE` 裡首頁、頁面設定、常見問題都是 `null`（永遠開著，
不對應任何賓客頁面）。把頁面設定搬出首頁的理由只有一個：埋在五步上手指南
下面的東西，**要用的時候找不到** —— 婚禮當天要把抽卡打開的人，
不會記得它在首頁最底下。

**抽屜底部只留「登出」**：`<900px` 時側欄是抽屜，「查看網站」頂列一直看得到、
email 是「我用哪個帳號登入」（屬於帳號選單），兩者都不必在抽屜裡再出現一次。

**「頁面設定」分頁**（`sites.pagePublish`，見第 10 節）：
一列一頁，**頁名 ＋ 緊跟在它右邊的「排程開啟」**在左、
決定賓客現在看不看得到的 toggle 在右。排程可以設一個時間讓它自己開
（`<input type="datetime-local">`，存成 epoch ms）。
清單是從 `site-context.js` 的 `PAGES` 長出來的，
**之後新增的頁面會自己出現在這一區**，不必回來改一份清單。

一列裡**刻意只有這三樣東西**：十來頁排下來，每一列多一行字就多滾一整頁。
所以每一頁那句「這一頁在做什麼」拿掉了（頁名自己說得夠清楚），
狀態那一行也只有**排了程的那幾列**才長出來
（`pageStateText()`）—— 開著沒開著 toggle 自己就看得出來，
唯一沒別的地方可以講的只有「幾點會自己開」。

| 這一列 | 怎麼來的 | 新人看到 |
|---|---|---|
| 我們沒開通的頁面 | `isPageOn(key) === false` | 灰的、掛鎖頭、頁名右邊是一顆問號（排程的位置）、toggle **預設關著**且 `disabled`（按下去會說要找誰，見下面） |
| 還沒對外開放的功能 | 上面那個 ＋ `UNRELEASED_FEATURES` | 整列不出現（和側欄的 `off` 同一個判斷） |
| 已經開通的頁面 | 其餘 | toggle 可按，排了程才多一行「幾點自動開啟」 |

右邊那支**手機示意**（`.ad-phone`，標題就寫 `Menu`）列的是賓客現在
打開首頁看得到的入口 —— **收起來的頁面直接不出現**：那支手機就是賓客的畫面，
...
每一類一個小標 ＋ 一疊 `<details>`（還是不做自己的手風琴）。
最後一塊 `.ad-support` 是官方帳號：這一頁回答不了的，讓他一鍵去問人。
官方帳號的網址**全站只寫在 `admin.js` 的 `SUPPORT_LINE_URL` 一個地方**
（常見問題那顆按鈕與上面那塊鎖頭說明讀的都是它），換帳號只改那一行。
...
「**出席表單設定**」與「**設定賓客標籤**」原本是出席回覆底下的第二、三顆
子分頁，實測時新人找不到 —— 他要找的是「我的表單長什麼樣」「名單怎麼分類」，
那都不是「已經收到的回覆」的一部分。現在它們各自是側欄「婚禮內容」裡的一頁。

「**婚禮資訊**」原本有四顆子分頁（婚禮資訊／其他流程／當日流程／自訂內容），
新人要填的其實就是「首頁上那一頁」，切來切去只是在找欄位。現在：
自訂內容是側欄自己的一頁，其餘三件事收成同一頁的幾個段落 ——
**標題 → 婚禮 hashtag → 婚禮流程（活動與當日流程）→ 交通資訊 → Dress Code
→ 關於禮金 → 兩人的故事 → 相遇之間**，由上到下就是賓客讀到的順序。

搬過家的舊網址由 `admin.js` 的 `LEGACY_HASH` 帶到新的位置
（`#rsvp/form` → `#rsvpForm`、`#rsvp/tags` → `#guestTags`、
`#lobby/*` → `#lobby`，`#lobby/explore` → `#explore`），
寄出去的信與新人的書籤都不會斷。
...
婚禮流程的活動資訊與自訂題目）：清單是這些分頁的主畫面，
表單只在要動它的時候才出現。活動資訊那一個彈窗兩個地方共用 ——
「婚禮資訊 → 婚禮流程」的活動卡與「出席表單設定」的活動卡，
改完寫的是同一份 `events[]`。
...
開關**只有「頁面設定」那一列**（見第 10 節）：「桌次」分頁上不再放
第二顆意思一樣的。「幾點自動開啟」的排程存在 `pagePublish.seating.at` ——
婚禮當天早上自己打開，新人不必記得回來按。
「排桌管理」同步完之後，如果桌次還關著，提示裡那顆「現在打開」
就直接跳到「頁面設定」的那一列並閃一下（`seating-plan.js`）。

**搜尋可以單獨關掉**（後台「桌次 → 桌次搜尋及名單」最上面那顆 switch
「讓賓客輸入名字查詢桌次」→ `seatingSearchEnabled`）：
...
### 13.3.1 Moments 相遇之間（首頁上自己的一段）

照片**不放在 Explore 裡**。Explore 是「還可以去哪裡」，照片不是另一個
可以去玩的活動，它就是這場婚禮本身；做成一張卡，等於要賓客先猜到
那張卡後面有東西才看得到。所以它是首頁上自己的一段（`index.js` 的
`renderPhotoGallery()`），三個版型都有：

- 位置：兩人的故事之後、日期倒數之前（`#photoGallery` ／ `.photo-rail`）
- 照片就是站台的 `photos`（沒填的話是素材資料夾 `gallery/` 掃到的那些）；
  一張都沒有就整段 `hidden`，不留一個空標題
- 版面：CSS `columns:2` 的兩欄錯落（窄螢幕一欄）。每一格的比例由
  `.pr-frame` 的 `aspect-ratio` 四張一循環先定好 —— 版面在圖片下載完
  之前就排好了，照片一張一張進來時整頁不會跳動
- **邊捲邊載**：`src` 一開始不填，`loadIO`（`rootMargin:600px`）捲到附近
  才補上。一進大廳不會同時發出十幾個圖片請求，那段頻寬留給封面與背景
- **漸進式進場**：`revealIO`（`threshold:.12`）真的進到畫面裡才加 `.is-in`
  淡入 ＋ 上浮，同一批進來的每張錯開 90ms；圖片自己載好時再加 `.is-loaded`
  從微微放大回到原尺寸。沒有 `IntersectionObserver` 的瀏覽器直接全部
  載好顯示 —— 動畫是加分的，照片看不看得到不能賭在它身上
- `prefers-reduced-motion` 不在這裡判斷：`common.css` 那條全站規則
  會把 transition 壓成 1ms，等於直接到位
- 點任何一張用既有的彈窗原地放大（`openInfoModal()`，單張），
  不另外做一套燈箱
...
卡池的來源優先序：`cards` 集合 → `assets/{slug}/cards/`，
**全有或全無**，不會混在一起。

**沒有內建範例卡**：兩邊都沒有圖時卡池就是空的 —— `#drawBtn` 停用，
底下的 `#drawEmpty` 寫「等待新人上傳照片」。原本 `js/draw.js` 裡寫死了
52 張某一組新人的生活照當退路，結果是每個新開的站台都會抽到別人的照片；
與其給一組不屬於這對新人的卡，不如老實說還在等新人上傳。
那行字要等 `cards` 的第一份 snapshot 回來（或被規則擋下）才寫出去 ——
在那之前的「卡池是空的」是還沒讀到，不是新人沒上傳。

**抽卡收藏為什麼多一個 `cardId`**：`collected.art` 的長度上限是 300 字元
（規則層擋著，避免每抽一張就寫進一份大文件）。
新人上傳的卡圖是整段 data URL，塞不進去，
所以收藏只記 `cardId`，畫面再回 `cards` 取圖。
素材資料夾的卡沿用原本的 `art`，舊資料不受影響。
