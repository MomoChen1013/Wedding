...
寫進同一個 `rsvps` 子集合、後台也只有一份儀表板。
題目各寫一份的話，一邊改了另一邊就對不上，後台會拿到兩種形狀的資料。

所以表單的 DOM 與送出邏輯集中在 **`public/js/rsvp-form.js`**，
由兩頁的 HTML 各放一個 `<div id="rsvpFormHost">` 掛上去。
...
後台儀表板讀的是同一份，圖表標籤才不會和賓客看到的選項對不起來。
