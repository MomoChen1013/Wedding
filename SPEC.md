...
2. **開始製作**（`HOME_STEPS`）：有順序的幾步，每一步右邊一顆
...
3. **給你的製作筆記**（`.ad-note`）：一張 `<details>`，收起來只有標題與副標題，
   展開是建議的節奏（第一週／慢慢做／前一到兩週／前一天／結束後）。
   第 2 點講「做什麼」，這一段講「什麼時候做」——
   一整份時程攤開在首頁比十顆分頁名稱更嚇人，所以預設收著。
