...
| 出席回覆 | `rsvp` |
| 婚禮資訊 | 永遠都在（大廳是必開的頁面） |
| 桌次 | `seating`（只看 `pages`：新人自己在「頁面設定」把桌次收起來時，後台照樣進得去） |
| 感謝信 | `letter` |
...
**側欄最下面的兩顆分頁**（`.ad-navbot`）：頁面設定與常見問題和「首頁」同一層
（都不是在編輯某一塊賓客內容），所以不進任何一個分組，用同一道淡線隔在最底下。
`TAB_PAGE` 裡兩者都是 `null`（永遠開著，不對應任何賓客頁面）。
搬出首頁的理由只有一個：埋在「開始製作」那幾步下面的東西，
**要用的時候找不到** —— 婚禮當天要把抽卡打開的人，不會記得它在首頁最底下。
...
| 婚禮資訊 | 婚禮資訊／其他流程／當日流程／自訂內容（含「相遇之間」的開關） |
| 桌次 | 桌次圖／桌次搜尋及名單 |
